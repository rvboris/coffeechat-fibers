...
module.exports = function (argv) {
    sync(function () {
        ...
        Loader.prototype.elasticSearch = function () {
            app.set('log').debug('setup elasticsearch');
                    ...
                    analysis: {
                        analyzer: {
                            'myAnalizer': {
                                type: 'custom',
                                tokenizer: 'standard',
                                filter: ['lowercase', 'russian_morphology', 'english_morphology', 'myStopwords']
                            }
                        },
                        filter: {
                            'myStopwords': {
        ...
        Loader.prototype.syncEvents = function (tKey) {
            ...
            starter.on('channels', function (channels, syncServer) {
                sync(function () {
                    ...
                    if (channels.length === 0) {
                        app.set('log').error('channels not found');
                        return;
                    }
            ...
            starter.on('users', function (users, syncServer) {
                sync(function () {
                    ...
                    if (users.length === 0) {
                        app.set('log').error('users not found');
                        return;
                    }
